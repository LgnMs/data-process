...
use crate::utils::{format_body_string, format_cron};
    ...
    pub async fn save(
        ...
        if let Some(id) = id {
            ...
            if db_data.cache_table_name != data.cache_table_name {
                Self::update_table_struct(cache_db, None, data.cache_table_name.as_ref().unwrap())
                    .await?;
            }
