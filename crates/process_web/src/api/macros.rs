...
macro_rules! bool_response {
    ($res: expr) => {
        match $res {
            ...
            Err(err) => {
                Ok(Json(res_template_err!(Some(false), err.to_string())))
            }
        }
    }
}
macro_rules! data_response {
    ($res: expr) => {
        match $res {
            Ok(data) => {
                Ok(Json(res_template_ok!(Some(data))))
            }
            Err(err) => {
                Ok(Json(res_template_err!(None, err.to_string())))
            }
        }
    }
}
...
macro_rules! pagination_response {
    ($res: expr, $current: expr, $page_size: expr) => {
        match $res {
            Ok((list, total)) => {
                Ok(Json(res_template_ok!(list, $current, $page_size, total)))
            }
            Err(err) => {
                Ok(Json(res_template_err!(vec![], $current, $page_size, 0, err.to_string())))
            }
        }
    }
}
